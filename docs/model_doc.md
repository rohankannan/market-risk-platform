...
Each instrument maps to exactly one factor (SPY to its own price series, the
10Y proxy to the 10Y constant-maturity yield). There is no tenor
interpolation, no key-rate decomposition, and no basis between an instrument
and its factor (§5).
...
2. **One factor per instrument; no tenor interpolation.** Each bond proxy
   ...
   Adequate for a five-point curve book; wrong for real curve trading.
